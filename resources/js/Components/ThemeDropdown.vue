<template>
  <div title="Change Theme" class="dropdown dropdown-end" :class="dropdownClasses">
    <div tabindex="0" class="btn gap-1 normal-case" :class="btnClasses">
      <svg width="20" height="20" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
        class="inline-block h-5 w-5 stroke-current md:h-6 md:w-6">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
          d="M7 21a4 4 0 01-4-4V5a2 2 0 012-2h4a2 2 0 012 2v12a4 4 0 01-4 4zm0 0h12a2 2 0 002-2v-4a2 2 0 00-2-2h-2.343M11 7.343l1.657-1.657a2 2 0 012.828 0l2.829 2.829a2 2 0 010 2.828l-8.486 8.485M7 17h.01" />
      </svg>
      <span class="hidden md:inline">Theme Change<!--{{ $t('ui.change-theme-btn') }}--></span>
    ...
    <div
      class="scrollbar-thin scrollbar-thumb-primary scrollbar-track-secondary dropdown-content bg-base-200 text-base-content rounded-t-box rounded-b-box top-px max-h-96 h-[70vh] w-52 overflow-y-auto shadow-2xl"
      :class="contentClasses">
      <div class="grid gap-3 p-3" tabindex="0">
        <div v-for="theme in themes" :key="theme.id">
          <div class="outline-base-content overflow-hidden rounded-lg outline outline-2 outline-offset-2"
            :data-set-theme="theme.id" data-act-class="outline" @click="themeSelected(theme.id)">
        ...
        <a class="outline-base-content overflow-hidden rounded-lg" href="/theme-generator/">
          <div class="hover:bg-neutral hover:text-neutral-content w-full cursor-pointer font-sans">
            <div class="flex gap-2 p-3">
              <svg width="24" height="24" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 fill-current"
                viewBox="0 0 512 512">
              ...
              <!-- <div class="flex-grow text-sm font-bold">{{ $t("ui.Make your theme") }}</div> -->
            </div>
          </div>
        </a>
      </div>
...
import { themes } from "@composables/useThemes.js"
...
function themeSelected(theme) {
  mainStore.setuiThemeColors(uiColors(theme))
}
